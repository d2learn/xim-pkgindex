...
function xpkg_main(gcc_bin, ...)
    ...
    -- printinfo
    log.debug("")
    log.debug("\tgcc-specs-config - 0.0.1")
    log.debug("")
    log.debug("---")
    log.debug("gcc: ${green}%s${clear}", gcc_bin)
    log.debug("dynamic-linker: ${green}%s${clear}", cmds["--dynamic-linker"])
    log.debug("runpath(rpath): ${green}%s${clear}", cmds["--rpath"])
    log.debug("---")
    log.debug("")
