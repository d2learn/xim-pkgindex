...
local function get_scriptdir()
    return system.xpkgdir()
end

local function get_task_list_file()
    return path.join(get_scriptdir(), "git-autosync-tasks.json")
end

local function get_crontab_file()
    return path.join(get_scriptdir(), "git-autosync-crontab")
end

local function get_log_file()
    return path.join(get_scriptdir(), "git-autosync.log")
end

local function get_git_autosync_sh_file()
    return path.join(get_scriptdir(), "git-autosync.sh")
end
...
PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
    ...
    if not os.isfile(get_log_file()) then
        io.writefile(get_log_file(), "")
    end

    if not os.isfile(get_task_list_file()) then
        io.writefile(get_task_list_file(), "{}")
    end
end
...
function to_crontab_task(cmds)
    ...
    if os.isdir(path.join(cmds["--project-dir"], ".git")) then
        task = string.format("\n%s %s %s %s %s >> %s 2>&1\n",
            ...
            path.join(get_scriptdir(), "git-autosync.sh"), cmds["--project-dir"], cmds["--post-command"] or "",
            get_log_file()
        )
    end 
...
function sync_to_system()
    ...
    system.exec(string.format("sudo cp -f %s /etc/cron.d/git-autosync-crontab", get_crontab_file()))
...
function action_add(cmds, task_list)
    ...
    if not task_list then
        task_list = json.loadfile(get_task_list_file()) or {}
    end
